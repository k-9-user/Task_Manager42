...
def lookup_user_by_email(
    ...
    """Exact-match email lookup so any project member can find a `user_id`
    to invite — unlike `list_users` (admin-only), this is open to every
    authenticated user but only ever returns a single exact match, never a
